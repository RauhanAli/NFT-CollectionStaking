...
contract NFTStaking is Ownable, IERC721Receiver{
    ...
    //struct to store stake NFT, owner and earning value
    ...
    struct Stake{
     ...
     constructor(Reward _token, NFT _nft){
        token = _token;
        nft = _nft;
     }
     ...
     function unstake(address account, uint256[] calldata tokenIds) internal{
        ...
        for(uint i = 0; i <tokenIds.length; i++){
            ...
            require(staked.owner == msg.sender, "NFT not belongs to You");
        ...
        function claim(uint256[] calldata tokenIds) external {
            Claim(msg.sender, tokenIds, false);
        }

        function claimForAddress(address account, uint256[] calldata tokenIds) external {
            Claim(account, tokenIds, false);
        }

        function unStake(uint256[] calldata tokenIds) external {
            Claim(msg.sender, tokenIds, true);
        }
     function Claim(address account, uint256[] calldata tokenIds, bool _unstake)internal {
        uint256 tokenId;
        uint256 earned = 0;
        for(uint i = 0; i <tokenIds.length; i++ ){
            tokenId = tokenIds[i];
            Stake memory staked = vault[tokenId];
            require(staked.owner == account, "NOT the Owner");
            uint256 StakeAt = staked.timestamp;
            earned += 1000 ether * (block.timestamp - StakeAt) / 1 days;
            vault[tokenId] = Stake({
                owner: account,
                tokenId : uint24(tokenId),
                timestamp: uint48(block.timestamp)
